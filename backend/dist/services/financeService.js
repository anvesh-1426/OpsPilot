...
exports.financeService = {
    ...
    getFinancialDashboard: async () => {
        const [totalIncome, totalExpenses, unpaidInvoices, unpaidPOs] = await Promise.all([
            ...
            prisma_1.default.invoice.findMany({ where: { status: { not: 'PAID' } }, select: { total: true } }),
            prisma_1.default.purchaseOrder.findMany({ where: { status: { not: 'RECEIVED' } }, select: { total: true } }),
        ]);
        ...
        const inventoryValuation = (await prisma_1.default.inventory.findMany({ include: { product: true } }))
